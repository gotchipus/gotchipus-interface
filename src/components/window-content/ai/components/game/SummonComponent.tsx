...
const SummonComponent = observer(({ onSummonSuccess }: SummonComponentProps) => {
  ...
  const {contractWrite, hash, isConfirmed, isConfirming, isPending, error, receipt} = useContractWrite();
  ...
  const handleSummon = () => {
    if (!pusName || !stakeAmount || selectedToken === null || !selectedGotchi) return;
    setIsSummoning(true);
    ...
    const args = [
      ...
      newTimezone,
      ethers.hexlify(ethers.toUtf8Bytes("Generated by AI"))
    ];
  ...
  useEffect(() => {
    if (isConfirmed && selectedGotchi) {
      ...
      if (onSummonSuccess && receipt?.transactionHash) {
        onSummonSuccess(selectedGotchi.id, receipt.transactionHash);
      }
          ...
          <div className="w-1/2 space-y-2">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <h3 className="text-sm font-bold">Stake {stakeToken} Token</h3>
